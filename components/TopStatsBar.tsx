...
const StatBox: React.FC<StatBoxProps> = ({ label, count, color, icon: Icon, active }) => (
  <div className={`${color} text-white rounded p-1.5 flex items-center gap-2 shadow-sm min-w-[100px] relative overflow-hidden flex-1 group hover:brightness-110 cursor-pointer transition-all ${active ? 'ring-2 ring-white ring-offset-1 scale-105 z-10' : ''}`}>
    ...
    {/* Floating notification badge style as seen in screenshot */}
...
export const TopStatsBar = () => {
  const stats = [
    { label: 'Jemaat', count: 19, color: 'bg-slate-600', icon: Users, active: true },
    { label: 'Jadwal', count: 5, color: 'bg-emerald-600', icon: Calendar },
    { label: 'Daftar', count: 6, color: 'bg-rose-600', icon: List },
    { label: 'Komsel', count: 6, color: 'bg-amber-500', icon: UsersRound },
    { label: 'Talenta', count: 4, color: 'bg-teal-500', icon: Star },
    { label: 'Penyerahan', count: 4, color: 'bg-slate-400', icon: UserPlus },
