...
const INITIAL_DATA: PelayananData[] = [
  ...
  { id: '2', namaPelayanan: 'Diaken', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '26 November 2022', dibuatJam: '00:03:10am' },
  { id: '3', namaPelayanan: 'Penerima Tamu', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '26 November 2022', dibuatJam: '00:03:10am' },
  { id: '4', namaPelayanan: 'Song Leader', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '26 November 2022', dibuatJam: '00:03:10am' },
  { id: '5', namaPelayanan: 'Singer', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '26 November 2022', dibuatJam: '00:03:10am' },
  { id: '6', namaPelayanan: 'Multimedia', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '26 November 2022', dibuatJam: '00:03:10am' },
];
...
export const DaftarPelayanan: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const renderBreadcrumb = (currentAction?: string) => (
    <div className="text-[11px] text-slate-400 font-bold uppercase tracking-widest text-right flex-1">
      MASTER DATA / <span className="text-blue-500 cursor-pointer hover:underline" onClick={() => setMode('list')}>Daftar Pelayanan</span> 
  ...
  const renderListView = () => (
    <div className="p-6 bg-slate-100 min-h-full">
      <TopStatsBar />
      {renderTopNav('Daftar Pelayanan')}
      
      <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-sm font-bold text-slate-700">Data Daftar Pelayanan</h2>
          <button 
          ...
          >
            <Plus size={16} /> Add Daftar Pelayanan
          </button>
        </div>
            ...
            <table className="w-full text-[11px] text-left border-collapse">
              <thead className="bg-slate-50 border-y border-slate-200 text-slate-700 font-bold">
                <tr>
                  ...
                  <th className="px-3 py-3 border-r border-slate-200 text-center">Nama Pelayanan</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length > 0 ? filteredData.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-3 py-4 border-r border-slate-200 text-slate-600 text-center">{item.namaPelayanan}</td>
                    ...
                    <td className="px-3 py-4 text-center whitespace-nowrap">
                      <div className="flex justify-center gap-1">
                        ...
                        <button onClick={() => handleDelete(item.id, item.namaPelayanan)} className="p-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded shadow-sm" title="Hapus Data"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                )) : (
  ...
  const renderFormView = (isEdit: boolean) => (
    <div className="p-6 bg-slate-100 min-h-full">
      <TopStatsBar />
      {renderTopNav(isEdit ? 'Edit Daftar Pelayanan' : 'Add Daftar Pelayanan', isEdit ? 'Edit Daftar Pelayanan' : 'Add Daftar Pelayanan')}
      
      <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-sm font-bold text-slate-700">{isEdit ? 'Edit Daftar Pelayanan' : 'Add Daftar Pelayanan'}</h2>
          <button 
        ...
        <div className="p-6">
          ...
          <div className="max-w-4xl space-y-5">
            <FormField label="Nama Pelayanan" required>
              <input type="text" className="form-input" defaultValue={selected?.namaPelayanan} placeholder="Nama Pelayanan" />
            </FormField>
  ...
  const renderDetailView = () => (
    <div className="p-6 bg-slate-100 min-h-full">
      <TopStatsBar />
      {renderTopNav('Detail Data Daftar Pelayanan', 'Detail Data Daftar Pelayanan')}
      
      <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-sm font-bold text-slate-700">Detail Data Daftar Pelayanan</h2>
          <button 
            onClick={() => setMode('list')} 
            className="bg-slate-700 hover:bg-slate-800 text-white px-4 py-1.5 rounded text-[11px] font-bold flex items-center gap-2 shadow-sm transition-all"
          >
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 gap-y-3 max-w-2xl">
            <DetailRow label="Nama Pelayanan" value={selected?.namaPelayanan} />
