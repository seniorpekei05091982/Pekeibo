...
const INITIAL_DATA: KomselData[] = [
  { 
    id: '1', 
    namaKomsel: 'Petrus', 
    keterangan: '-',
    dibuatOleh: 'admin',
    dibuatTanggal: '06 Februari 2023',
    dibuatJam: '13:46:37pm',
    diubahOleh: '-',
    diubahTanggal: '06 Februari 2023',
    diubahJam: '13:46:37pm'
  },
  { id: '2', namaKomsel: 'Paulus', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '06 Februari 2023', dibuatJam: '13:46:37pm' },
  { id: '3', namaKomsel: 'Abraham', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '06 Februari 2023', dibuatJam: '13:46:37pm' },
  { id: '4', namaKomsel: 'Belum Mengikuti', keterangan: '-', dibuatOleh: 'admin', dibuatTanggal: '06 Februari 2023', dibuatJam: '13:46:37pm' },
];

type ViewMode = 'list' | 'add' | 'edit' | 'detail';

export const Komsel: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const [data, setData] = useState<KomselData[]>(INITIAL_DATA);
  const [mode, setMode] = useState<ViewMode>('list');
  const [selected, setSelected] = useState<KomselData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeTab, setActiveTab] = useState('Komsel');

  const filteredData = useMemo(() => {
    return data.filter(item => 
      item.namaKomsel.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [data, searchTerm]);

  const handleDelete = (id: string, name: string) => {
    if (window.confirm(`Hapus data Komsel "${name}"?`)) {
  ...
  const renderBreadcrumb = (currentAction?: string) => (
    <div className="text-[11px] text-slate-400 font-bold uppercase tracking-widest text-right flex-1">
      MASTER DATA / <span className="text-blue-500 cursor-pointer hover:underline" onClick={() => setMode('list')}>Komsel</span> 
    ...
    <div className="p-6 bg-slate-100 min-h-full">
      ...
      <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-sm font-bold text-slate-700">Data Komsel</h2>
          <button 
            ...
            <table className="w-full text-[11px] text-left border-collapse">
              <thead className="bg-slate-50 border-y border-slate-200 text-slate-700 font-bold">
                <tr>
                  ...
                  <th className="px-3 py-3 border-r border-slate-200 text-center">Nama Komsel</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length > 0 ? filteredData.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-3 py-4 border-r border-slate-200 text-slate-600 text-center font-medium">{item.namaKomsel}</td>
                    ...
                    <td className="px-3 py-4 text-center whitespace-nowrap">
                      <div className="flex justify-center gap-1">
                        ...
                        <button onClick={() => handleDelete(item.id, item.namaKomsel)} className="p-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded shadow-sm" title="Hapus Data"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                )) : (
  ...
  const renderFormView = (isEdit: boolean) => (
    <div className="p-6 bg-slate-100 min-h-full">
      <TopStatsBar />
      {renderTopNav(isEdit ? 'Edit Komsel' : 'Add Komsel', isEdit ? 'Edit Komsel' : 'Add Komsel')}
      
      <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-sm font-bold text-slate-700">{isEdit ? 'Edit Komsel' : 'Add Komsel'}</h2>
          <button 
        ...
        <div className="p-6">
          ...
          <div className="max-w-4xl space-y-5">
            <FormField label="Nama Komsel" required>
              <input type="text" className="form-input" defaultValue={selected?.namaKomsel} placeholder="Nama Komsel" />
            </FormField>
  ...
  const renderDetailView = () => (
    <div className="p-6 bg-slate-100 min-h-full">
      <TopStatsBar />
      {renderTopNav('Detail Komsel', 'Detail Data Komsel')}
      
      <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-sm font-bold text-slate-700">Detail Data Komsel</h2>
          <button 
            onClick={() => setMode('list')} 
            className="bg-slate-700 hover:bg-slate-800 text-white px-4 py-1.5 rounded text-[11px] font-bold flex items-center gap-2 shadow-sm transition-all"
          >
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 gap-y-3 max-w-2xl">
            <DetailRow label="Nama Komsel" value={selected?.namaKomsel} />
